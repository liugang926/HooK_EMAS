    ...
    <el-card class="page-card">
      ...
      <el-form :inline="true" class="filter-form">
        <el-form-item label="Order No">
          <el-input v-model="filterForm.inbound_no" placeholder="Enter" clearable />
        </el-form-item>
      ...
      <el-table :data="inboundList" style="width: 100%" v-loading="loading">
        <el-table-column prop="inbound_no" label="Order No" width="160" />
        <el-table-column prop="warehouse_name" label="Warehouse" width="120" />
        <el-table-column prop="supplier_name" label="Supplier" width="150" />
        <el-table-column prop="inbound_date" label="Date" width="120" />
    ...
    <el-dialog v-model="formDialogVisible" :title="formDialogTitle" width="900px" destroy-on-close>
      <el-form :model="inboundForm" label-width="100px" ref="formRef">
        <el-row :gutter="20">
          <el-col :span="8">
            <el-form-item label="Warehouse" prop="warehouse">
              <el-select v-model="inboundForm.warehouse" placeholder="Select" style="width: 100%">
                <el-option v-for="item in warehouseOptions" :key="item.id" :label="item.name" :value="item.id" />
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="Date" prop="inbound_date">
              <el-date-picker v-model="inboundForm.inbound_date" type="date" value-format="YYYY-MM-DD" style="width: 100%" />
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="Supplier">
              <el-select v-model="inboundForm.supplier" placeholder="Select" style="width: 100%" clearable filterable>
        ...
        <el-form-item label="Remark" style="margin-top: 16px">
          <el-input v-model="inboundForm.remark" type="textarea" :rows="2" placeholder="Enter remark" />
        </el-form-item>
      </el-form>
      <template #footer>
...
const formDialogTitle = ref('Add Inbound')
...
function handleAdd() {
  formDialogTitle.value = 'Add Inbound'
...
function handleView(row) { ElMessage.info('View inbound: ' + row.inbound_no) }
function handleEdit(row) { formDialogTitle.value = 'Edit Inbound'; formDialogVisible.value = true }

async function handleApprove(row) {
  try {
    await ElMessageBox.confirm('Confirm inbound will update stock. Continue?', 'Confirm', { type: 'warning' })
...
async function handleDelete(row) {
  try {
    await ElMessageBox.confirm('Delete this inbound order?', 'Confirm', { type: 'warning' })
...
async function submitForm() {
  if (!inboundForm.warehouse) { ElMessage.warning('Please select warehouse'); return }
  if (inboundForm.items.some(i => !i.consumable)) { ElMessage.warning('Please select supply'); return }
